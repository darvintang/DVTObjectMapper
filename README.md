...
# 安装
### Cocoapods
如果你的项目使用`CocoaPods`，你可以把下面内容添加到在 `Podfile` 中，将 ObjectMapper 添加到你的项目中:

```ruby
pod 'DVTObjectMapper', '~> 14.0'
```

### Swift Package Manager
如果你的项目使用  [Swift Package Manager](https://swift.org/package-manager/) ，那么你可以把下面内容添加到 `Package.swift` 中的 `dependencies` 数组中，将 DVTObjectMapper 的依赖到你的项目中：

```swift
.package(url: "https://github.com/darvintang/DVTObjectMapper.git", .upToNextMajor(from: "14.0.0"))
```

原项目链接：[ObjectMapper](https://github.com/tristanhimmelman/ObjectMapper)

原项目LICENSE：[LICENSE](https://github.com/tristanhimmelman/ObjectMapper/blob/master/LICENSE)
